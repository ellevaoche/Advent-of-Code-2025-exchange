# %%
inp0 = """Game 1: 3 blue, 4 red; 1 red, 2 green, 6 blue; 2 green
...
inp1 = """Game 1: 4 blue, 4 red, 16 green; 14 green, 5 red; 1 blue, 3 red, 5 green
...
num_col_d = {"red": 12, "green": 13, "blue": 14}

game1_l = [line.split(":") for line in inp.splitlines()]
game2_l = [(int(game.split(" ")[1]), moves.split(";")) for game, moves in game1_l]

sum_id = 0
for game_id, moves_l in game2_l:
    possible = True
    balls_l = [move.split(",") for move in moves_l]
    print(f"Game {game_id}")
    for move_num, balls in enumerate(balls_l):
        for entry in balls:
            num_str, color = entry.strip().split(" ")
            num = int(num_str)
            # print(f"        {num}, {color}")
            if num > num_col_d[color]:
                print(f"    Move {move_num+1}")
                print(f"        {num} {color}: IMPOSSIBLE, > {num_col_d[color]}")
                possible = False
    if possible:
        sum_id += game_id

print(f"Result: {sum_id}")
