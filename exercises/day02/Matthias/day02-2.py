...
for game_id, moves_l in game2_l:
    min_balls_d = {"red": 0, "green": 0, "blue": 0}

    balls_l = [move.split(",") for move in moves_l]
    for balls in balls_l:
        for entry in balls:
            num_str, color = entry.strip().split(" ")
            num = int(num_str)
            if num > min_balls_d[color]:
                min_balls_d[color] = num
    power = functools.reduce(operator.mul, min_balls_d.values())
    sum_power += power
