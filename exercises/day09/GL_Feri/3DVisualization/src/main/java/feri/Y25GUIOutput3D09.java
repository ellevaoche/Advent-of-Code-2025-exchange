...
public class Y25GUIOutput3D09 extends Application {

	static Y25GUIOutput3D09 instance = null;
	
	public static int ANIMATION_SPEED_MS = 250;
	...
	public static class DDDBlockObject extends DDDObject {
		double x2;
		double y2;
		double z2;
		public DDDBlockObject(double x, double y, double z, double x2, double y2, double z2, double size, int type) {
			this(null, x, y, z, x2, y2, z2, size, type);
		}
		public DDDBlockObject(String id, double x, double y, double z, double x2, double y2, double z2, double size, int type) {
			super(id, x, y, z, size, type);
			this.x2 = x2;
			this.y2 = y2;
			this.z2 = z2;
	...
	public Y25GUIOutput3D09(String title, boolean useCachedNodes) {
		this.title = title;
    ...
    public Box createBlock(Point3D corner1, Point3D corner2, PhongMaterial mat) {
        // x axis vector is <1,0,0>
        // y axis vector is <0,1,0>
        // z axis vector is <0,0,1>
    	
        Point3D midpoint = corner1.midpoint(corner2);
        ...
        Box block = new Box(dx, dy, dz);

		block.setMaterial(mat);

        Translate moveToMidpoint = new Translate(midpoint.getX(), midpoint.getY(), midpoint.getZ());
        block.getTransforms().addAll(moveToMidpoint);
...
//        line.getTransforms().addAll(moveToMidpoint, rotateAroundCenter);
	...
	private void setBlockSizeAndPos(Node child, Point3D corner1, Point3D corner2) {
        Box block = (Box) child;
        ...
        Translate moveToMidpoint = (Translate)block.getTransforms().get(0);
        moveToMidpoint.setX(midpoint.getX());
        moveToMidpoint.setY(midpoint.getY());
        moveToMidpoint.setZ(midpoint.getZ());
	...
	private Node createNode(DDDObject dddo) {
		...
		case 40,41,42,43,44: {
			...
			DDDBlockObject block = (DDDBlockObject)dddo;
			Point3D from = new Point3D(scale*(block.x-offsetX), scale*(block.y-offsetY), scale*(block.z-offsetZ));
			Point3D to = new Point3D(scale*(block.x2-offsetX), scale*(block.y2-offsetY), scale*(block.z2-offsetZ));
			child = createBlock(from, to, mat);
			doTranslate = false;
			break;
		}
		default:
	...
	private void updateNode(Node child, DDDObject dddo) {
		...
		case 40,41,42,43,44: {
			...
			Point3D from = new Point3D(scale*(block.x-offsetX), scale*(block.y-offsetY), scale*(block.z-offsetZ));
			Point3D to = new Point3D(scale*(block.x2-offsetX), scale*(block.y2-offsetY), scale*(block.z2-offsetZ));
			setBlockSizeAndPos(child, from, to);
			doTranslate = false;
			break;
		}
		default:
