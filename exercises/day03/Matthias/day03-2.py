...
def main(inp: str, debug: bool = False):
    ...
    for line_num, line in enumerate(lines_l):
        for pos, char in enumerate(line):
            if char == "*":
                if debug:
                    print(f"Star at: {line_num}, {pos}")
                num_l = []
                for num_line_num, start, end in num_pos_l:
                    found = False
                    if num_line_num == line_num and pos in (start - 1, end):
                        found = True
                    elif abs(num_line_num - line_num) == 1 and start - 1 <= pos <= end:
                        found = True
                    if found:
                        num_l.append(int(lines_l[num_line_num][start:end]))
                if debug:
                    print(f"Numbers adjacent to star at {line_num}, {pos}: {num_l}")
                if len(num_l) == 2:
                    ratio = int(num_l[0]) * int(num_l[1])
                    sum_ratio += ratio
                    if debug:
                        print(f"Ratio: {ratio}")

    print(f"Result: {sum_ratio}")
